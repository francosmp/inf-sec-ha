...
class Body extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      root: 'https://97dc5f43.ngrok.io/inf-sec-php-ser/servicios-php.php',
      mirror: 'http://7f1be498.ngrok.io/inf-sec-php-ser/servicios-php.php',
      backup: '',
      data: [],
      index: '',
      codigo: ''
  ...
  postCreate() {
    if (this.state.codigo === '') {
      ...
      axios.post(this.state.root, { "crud": "create", "index": "0", "codigo": this.state.codigo })
        .then(res => {
  ...
  postRead() {
    axios.post(this.state.root, { "crud": "read", "index": "0", "codigo": "0" })
      .then(res => {
  ...
  postUpdate() {
    if (this.state.codigo === '' || this.state.index === '') {
      ...
      axios.post(this.state.root, { "crud": "update", "index": this.state.index, "codigo": this.state.codigo })
        .then(res => {
          this.setState({ index: '', codigo: '' }, function () { this.postRead(); }.bind(this));
        })
    }
  ...
  postDelete() {
    axios.post(this.state.root, { "crud": "delete", "index": "0", "codigo": "0" })
      .then(res => {
        this.postRead();
      });
  }
